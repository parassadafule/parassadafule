    ...
    >
      <div className="absolute inset-0 opacity-20">
        <div
          ...
          style={{
            backgroundImage:
              "repeating-linear-gradient(transparent, transparent 23px, #d1d5db 23px, #d1d5db 24px)",
      ...
      <div className="text-center z-10 px-4 sm:px-6">
        <div className="mb-6 sm:mb-8 flex justify-center">
          <div
            className={`w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-dashed border-gray-600 bg-white transition-all duration-1000 ${
              isVisible ? "animate-pulse" : ""
            }`}
          >
            <div className="w-full h-full rounded-full flex items-center justify-center relative">
              <div
                className={`absolute inset-2 border-2 border-gray-400 rounded-full transition-all duration-3000 ${
                  isVisible ? "opacity-100 scale-100" : "opacity-0 scale-50"
                }`}
              />
              <div
                className={`absolute inset-4 border border-gray-500 rounded-full transition-all duration-1000 delay-500 ${
        ...
        <h1
          className={`text-3xl sm:text-4xl md:text-6xl font-mono font-bold mb-4 text-gray-700 transition-all duration-1000 ${
        ...
        <div className="flex justify-center mb-4 sm:mb-6">
          <div
            className={`h-1 bg-gray-600 transition-all duration-1000 delay-1000 ${
              isVisible ? "w-48 sm:w-60" : "w-0"
            }`}
            style={{
              background:
                "repeating-linear-gradient(90deg, #6b7280, #6b7280 10px, transparent 10px, transparent 15px)",
            }}
          />
        </div>

        <h2
          className={`text-lg sm:text-xl md:text-2xl mb-4 sm:mb-6 text-gray-600 font-mono transition-all duration-1000 delay-700 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          }`}
        >
          Software Developer
        ...
        <div
          className={`max-w-2xl sm:max-w-3xl mx-auto mb-6 sm:mb-8 transition-all duration-1000 delay-1000 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          }`}
        >
      ...
      <div className="absolute inset-0 pointer-events-none">
        {[...Array(6)].map((_, i) => (
          ...
          >
            ...
            {i % 3 === 1 && <div className="w-4 h-8 border-l-2 border-gray-400 transform rotate-45" />}
            {i % 3 === 2 && (
